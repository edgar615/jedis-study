...
local max_sequence = tonumber(KEYS[1]) --自增序列的最大值
local min_logical_shard_id = tonumber(KEYS[2]) --最小的分片ID
local max_logical_shard_id = tonumber(KEYS[3]) --最大的分片ID
local num_ids = tonumber(KEYS[4]) --自增的数量

if redis.call('EXISTS', lock_key) == 1 then
  redis.log(redis.LOG_INFO, 'Cannot generate ID, waiting for lock to expire.')
...
if end_sequence >= max_sequence then
  ...
  --]]
  redis.log(redis.LOG_INFO, 'Rolling sequence back to the start, locking for 1ms.')
